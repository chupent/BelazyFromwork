...
public class MoneyFormatUtils {
     ...
     * @return
     */
    public String formatWanYuan(String moneyStr) {
//        checkPrecisionWanYuan(moneyStr);
        String result;
        result = convertToChineseNumber(moneyStr);
        result = addUnitsToChineseMoneyString(result);
        return result;
    ...
    private void checkPrecisionWanYuan(String moneyStr) {
        int fractionDigits = moneyStr.length() - moneyStr.indexOf(DOT) - 1;
        if (fractionDigits > 6) {
            //精度不能比分低
            throw new RuntimeException("金额" + moneyStr + "的小数位多于六位。");
        }
    }
